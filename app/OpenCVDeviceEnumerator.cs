...
public static class OpenCVDeviceEnumerator
{
    ...
    public static Camera[] EnumerateCameras()
    {
        ...
        var cameraNames = CameraEnumerator.Get();
        ...
        for (int drv = 0; drv < _drivers.Length; drv++)
        {
            ...
            for (int idx = 0; idx < MaxID; idx++)
            {
                ...
                if (cap.IsOpened())
                {
                    ...
                    camIDs.Add(new Camera(driverEnum + idx, cameraName));  // vector of all available cameras
                    cap.Read(frame);
                    if (frame.Empty())
                        System.Diagnostics.Debug.WriteLine($"{driverName}+{idx}\t opens: OK \t grabs: FAIL");
                    else
                        System.Diagnostics.Debug.WriteLine($"{driverName}+{idx}\t opens: OK \t grabs: OK");
                }
                cap.Release();
            }
        ...
        System.Diagnostics.Debug.WriteLine($"{camIDs.Count} camera IDs has been found ");

        return camIDs.ToArray();
    }
